       ...
       <div class="bg-white overflow-hidden shadow sm:rounded-lg">
          ...
          <div class="w-full overflow-hidden rounded-lg shadow-xs">
              ...
              <table class="w-full whitespace-no-wrap">
                <thead>
                  <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b bg-gray-50">
                    <th class="px-4 py-3">Link Category </th>
                    <th class="px-4 py-3">Links </th>
                    <th class="px-4 py-3 text-right">Actions</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y">
                  @foreach ($weblinkcategories as $item)
                  <tr class="text-gray-700">
                    ...
                    <td class="px-4 py-3">
                      <div class="flex items-center text-sm">
                        <a href="{{ route('admin.web.link', $item->id)}}" class="bg-green-400 px-4 py-2 rounded-full text-white text-sm">
                          <i class="fa fa-tag fa-lg mr-2"></i>Links ( {{ $item->weblink->count()}} )
                        </a>
                      </div>
                    </td>
            ...
            <div class="px-4 py-3 my-4 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t bg-gray-50 min-h-min">
              {{ $weblinkcategories->links()}}
            </div>
          </div>
        </div>
        <!-- Add/Edit Modal -->
        @include('livewire.admin.weblinkcategories.partials._add')
          <!--Delete Modal -->
        @include('livewire.admin.weblinkcategories.partials._delete')
      </div>
  </div>
</div>
