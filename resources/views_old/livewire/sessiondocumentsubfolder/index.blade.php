<div>
   <!-- Breadcrumb -->
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span class="ml-1 text-sm font-medium text-gray-700 md:ml-2">NA Sitting</span>
          </div>
        </li>
      </ol>
    </nav>
    ...
    <div x-data="{ sidebarOpen: false}">
            ...
            <div class="h-5 text-2xl text-center border-b-2 border-sky-500">
             ...
             <div class="flex flex-row items-center justify-start gap-4 my-2">
              <div class="my-2 text-sm"> <i class="mr-2 text-2xl text-yellow-500 fa fa-folder-open"></i>
                @isset($selectedSubFolder) {{$selectedSubFolder->name}} @endisset
              </div> 

              <div class="relative flex items-center justify-start group">
                <a href="{{ route('app.na.session.documents', [$this->sID, $this->catID])}}">
                  <i class="text-gray-700 fa fa-level-up-alt fa-lg"></i>
                </a>
                <div class="tool-tip">go back</div>
              </div>
             </div>
                          ...
                          <td class="p-4 px-6 text-xs border-t border-l-0 border-r-0 whitespace-nowrap">
                            <div class="flex items-center justify-end gap-2">
                                @if ($item->extension=='wav'||$item->extension=='mp3')
                                      ...
                                      @include('livewire.sessiondocumentsubfolder.partials._viewAudio')
                                  ...
                                  {{-- <button wire:click="showVideoModal()" class="px-6 py-2 text-xs font-bold border rounded-full border-cyan-500">
                                      ...
                                      @include('livewire.sessiondocumentsubfolder.partials._viewVideo') --}}
                                  @elseif($item->extension=='jpg'||$item->extension=='jpeg'||$item->extension=='png'||$item->extension=='gif'||$item->extension=='SVG')
                                      ...
                                      @include('livewire.sessiondocumentsubfolder.partials._viewImage')
                                  @else
                                  <div class="flex flex-row gap-2">
                                    ...
                                    @endif 
                                      ...
                                      @include('livewire.sessiondocumentsubfolder.partials._viewDoc')
                                  </div>
                                  @endif
      ...
      @include('livewire.sessiondocumentsubfolder.partials._add')
      @include('livewire.sessiondocumentsubfolder.partials._delete')
</div>
